...
| Picker availability wording | `cd frontend && npm.cmd test -- --run tests/cardPickerModal.test.ts`; `cd frontend && npm.cmd run build` | 2 passed plus frontend build passed; card/search picker badges now say `Selectable`/`Unavailable` instead of overclaiming full rules legality with a generic `Legal` badge. |
...
- Changed the search/card picker availability badge from `Legal`/`Illegal` to `Selectable`/`Unavailable` so the UI reflects the prompt filter instead of claiming global rules certainty.
