...
| Mana-value parser filters | `cd engine && npm.cmd test -- --run src/effects/parser.test.ts src/effects/continuous.test.ts src/authority.test.ts`; `cd engine && npm.cmd run build` | 257 focused engine tests passed plus engine build passed; generic library searches and static spell cost modifiers now parse `with mana value` constraints, including inclusive and strict less-than/greater-than wording, into executable CMC filters. |
...
- Added parser support for generic `with mana value` constraints on library searches and static spell-cost modifiers, including strict `less than` / `greater than` wording, so these effects now feed the existing authority/executor CMC filter path instead of dropping the restriction.
