...
| Named-card choice UI | `DECKREPS_BASE_URL=http://127.0.0.1:5179 node scripts/play_named_card_choice_ui_playtest.js` | Passed against fresh built preview; cast Tainted Pact through the browser, opened the "name a card" picker, selected Thassa's Oracle from the searchable card list, and resolved the stack. |
...
- Added `/play` browser support for live "name a card" stack choices, using the same searchable picker surface for Tainted Pact / Demonic Consultation style effects instead of relying on a deterministic named-card shortcut.
