...
| Manual commander zone replacement | `cd engine && npm.cmd test -- --run src/actions-public.test.ts src/commander.test.ts`; `cd engine && npm.cmd run build` | 80 passed plus engine build passed; manual move corrections now apply commander replacement, so sending a commander to graveyard/exile/hand through correction resolves to command zone by default. |
...
| Picker availability/destination wording | `cd frontend && npm.cmd test -- --run tests/cardPickerModal.test.ts`; `cd frontend && npm.cmd run build` | 3 passed plus frontend build passed; card/search picker badges now say `Selectable`/`Unavailable` instead of overclaiming full rules legality, and destination chips distinguish top/bottom of library and command zone movement. |
...
- Changed the search/card picker availability badge from `Legal`/`Illegal` to `Selectable`/`Unavailable` so the UI reflects the prompt filter instead of claiming global rules certainty, and expanded destination chips from broad `Top`/`Bottom`/`Command` labels to explicit library/command-zone destinations.
...
- Applied commander replacement to manual move corrections, keeping commanders out of graveyard/exile/hand when the trainer's default command-zone replacement is active.
