...
- Changed the search/card picker availability badge from `Legal`/`Illegal` to `Selectable`/`Unavailable` so the UI reflects the prompt filter instead of claiming global rules certainty, expanded destination chips from broad `Top`/`Bottom`/`Command` labels to explicit library/command-zone destinations, and made reveal/hidden picker state explicit.
