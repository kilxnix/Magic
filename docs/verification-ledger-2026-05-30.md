...
| Imported card type parsing | `cd engine && npm.cmd test -- --run src/cards/deck-loader.test.ts src/cards/card-parser-cache.test.ts src/__tests__/mana-colors.test.ts src/__tests__/mana-colors-comprehensive.test.ts`; `cd engine && npm.cmd run build` | 65 passed plus engine build passed; imported Scryfall type lines now derive card types from the pre-dash type section only, so subtype/name fragments such as `Island Scout` do not mark a creature as a land. |
