...
| 8.3 Attack Requirements And Restrictions | partial; `attacks each combat if able` is now enforced during attacker declaration and legal-action generation suppresses no-attack/optional-only attacks when a required attacker is actually able. Broader attack restrictions and requirements remain incomplete |
