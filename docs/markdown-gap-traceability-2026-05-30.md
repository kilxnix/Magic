...
| 3. Typed prompt lifecycle | partial | Search, library-choice, Brainstorm hand-to-library-top ordering, Fact or Fiction pile choice, and Ward pay/decline choices improved. No universal `ActionRequest` request-id/state-id system across all decisions. |
...
| 11. Mandatory choices separate from priority | partial | Search/scry/surveil choices exist, Brainstorm has explicit hand-to-library-top selection support, Fact or Fiction has explicit pile-selection support, and Ward has explicit pay/decline support. Universal mandatory-choice handling is not complete. |
...
| 2.12 Modal And Optional Choices | partial; Brainstorm now has explicit hand-to-library-top ordering, Fact or Fiction has explicit pile selection, and Ward has explicit pay/decline choice handling, but broad modal/optional choice UI is still incomplete |
...
| 5.3 No Unified Typed Prompt Surface | partial; `/play` now covers search, scry, surveil, sacrifice, library-top ordering, and typed named-card choice prompts, and the engine supports Fact or Fiction and Ward choice payloads, but the prompt system is still not one fully generic typed request surface across every rule/action path |
