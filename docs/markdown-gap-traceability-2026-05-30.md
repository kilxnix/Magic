...
| 2.7 Replacement And Prevention Effects | partial; delayed blink/flicker effects now exile immediately and return from exile through a one-shot next-end-step delayed trigger instead of using the previous immediate-return shortcut. Platinum Angel-style static outcome prevention and Everybody Lives-style turn-scoped prevention now cover life loss, game loss, game win prevention, save/load, and next-turn expiry. |
| 2.8 State-Based Actions | partial/test-proven; same-name legendary permanents can now use an explicit keep choice during SBAs instead of always keeping the first permanent, and the choice persists through save/load. |
...
| 2.12 Modal And Optional Choices | partial; Brainstorm now has explicit hand-to-library-top ordering, Fact or Fiction has `/play` pile selection and no longer falls back to a deterministic pile when a required stack choice is missing, Ward has `/play` pay/decline choice handling, parser-backed `choose one or both` modal spells accept one-or-both mode selections, and `choose one or more` spells can generate every non-empty mode set; broad modal/optional choice UI is still incomplete |
...
| 8.3 Attack Requirements And Restrictions | partial; `attacks each combat if able` is now enforced during attacker declaration and legal-action generation suppresses no-attack/optional-only attacks when a required attacker is actually able. Propaganda/Ghostly Prison-style per-attacker taxes are also paid during declaration and unaffordable taxed attacks are omitted from legal actions. Broader attack restrictions and requirements remain incomplete |
...
Additional stack-choice, turn-state, combat-requirement, replacement-order, game-outcome, and SBA-choice verification added after this audit: Jeska's Will now counts the chosen target opponent's hand for red mana and grants this-turn play permission to the exiled top three cards; Brainstorm supports explicit hand-to-library-top selection; Fact or Fiction supports `/play` pile selection from the revealed top cards and waits on the stack instead of choosing a deterministic pile when that required choice is missing; copy effects preserve copiable face/choice values while excluding damage/counters; Ward can be explicitly paid or declined from `/play`; parser-backed `choose one or both` modal spells accept one selected mode or both selected modes while rejecting empty/duplicate selections; `choose one or more` modal spells can select any non-empty mode set; Farewell no longer uses an exile-creatures-only shortcut and can exile all graveyards; Chart a Course now uses attacked-this-turn state instead of an always-discard shortcut; `attacks each combat if able` is enforced for able attackers; Propaganda-style attack taxes are charged during attacker declaration; multiple replacement effects can use persisted explicit order choices instead of always applying in registration order; Platinum Angel-style game-outcome prevention now stops life/poison/commander-damage loss, explicit lose-game effects, and opposing explicit win-game effects while the relevant static text is on the battlefield; and legend-rule SBAs can honor an explicit keep choice instead of always keeping the first copy.
