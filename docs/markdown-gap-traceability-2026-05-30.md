...
| Opening mulligan selection | partial/test-proven | `/play` selected-card mulligans now stay in redraw-selection mode through repeated mulligans and enter bottom-card selection only after the player keeps; broader multiplayer mulligan UX is not fully certified. |
...
| 2.3 Card Type Constraints | partial/test-proven; engine card filters, prompt failure reasons, imported card type parsing, and frontend deck/room/draft classification now match types, supertypes, and subtypes as separated type-line terms instead of broad substrings, with regression coverage for `land` not matching inside `Island`, subtype phrase matching such as `Time Lord`, and `Creature - Island Scout` not importing as a land. |
