...
| BCL-006 | Save/replay canonicality | `2f92efc` | QA save snapshots now seed `engineEventLogInitialState` from the starting engine state. Frontend regression passed for canonical SaveManager slot persistence. Live browser-extension retest on `?qa=complex-combat`: after resolving the complex combat damage prompt, saving Slot 1 displayed `SaveManager verified`, `SaveManager primary`, and `Audit OK (9)` instead of `Replay audit warning`; clicking `Load` restored the same board state with human life 44, defenders 39/36/38, opponent graveyard 2, and human graveyard 1. |
| BCL-002 | Scry/surveil/modal/tutor/replacement prompts | `25dfe1e` | Browser prompt matrix added in `docs/browser-prompt-matrix-2026-06-02.md`. The matrix ties live browser-extension proofs to replacement choices, fetch/search subtype filtering, Sisay legendary search predicates, modal modes, scry, surveil, spell targets, stack-object targets/copying, trigger ordering, and combat damage ordering. |
