/**
 * Shelector MTGGoldfish Duel: Commander precon vs Commander precon
 *
 * Uses MTGGoldfish deck download strings (often single-line exports) and runs a
 * headless AI-vs-AI duel for enough turns to expose game-loop failures.
 *
 * Note: This test uses stubbed card definitions (vanilla creatures + simple
 * mana lands) because the repo's full `mtg_data/cards_min.jsonl` is not
...
function makeStubLand(id: string, name: string, isBasic: boolean): ScryfallCard {
  ...
  // Generic land: colorless mana only (keeps the game moving with stubbed spells).
  return {
    id,
    name,
    type_line: 'Land',
...
function buildStubCardDatabase(deckA: { commander: string; cards: string[]; lands: Set<string> }, deckB: { commander: string; cards: string[]; lands: Set<string> }): ScryfallCard[] {
  ...
  for (const name of allNames) {
    ...
    const id = `stub-${idx++}`;
    if (isLand) cards.push(makeStubLand(id, name, isBasic));
    else cards.push(makeStubCreature(id, name));
  }
  return cards;
}

// ---------------------------------------------------------------------------
// Game simulation helpers (adapted from shelector-game-2)
// ---------------------------------------------------------------------------
...
describe('Shelector MTGGoldfish Duel: Precon vs Precon (stubbed)', () => {
  it('runs a headless AI-vs-AI duel for at least 20 half-turns without crashing', () => {
    ...
    // Ensure we parsed something sane.
    expect(deckAAll.length).toBe(100);
    expect(deckBAll.length).toBe(100);
    expect(deckAAll.includes(deckACommander)).toBe(true);
    expect(deckBAll.includes(deckBCommander)).toBe(true);

    // Remove commander from library (one copy).
    ...
    expect(deckALibrary.length).toBe(99);
    expect(deckBLibrary.length).toBe(99);

    // Land classification (explicit, deck-local) so the stubs have enough mana sources.
    ...
    const maxHalfTurns = 40; // 20 full turns max
    ...
    for (let halfTurn = 0; halfTurn < maxHalfTurns; halfTurn++) {
      ...
      for (const d of atk.decisions) {
        if (d.action.kind === 'DeclareAttackers') log.push(`    [COMBAT] ${activeId} declared attackers`);
      }
      ...
      for (const d of blk.decisions) {
        if (d.action.kind === 'DeclareBlockers') log.push(`    [COMBAT] ${defenderId} declared blockers`);
      }
      ...
      // FIRST STRIKE DAMAGE (no-op unless relevant)
      state = passPriority(state);
      state = passPriority(state);
      ...
      if (state.combat && state.combat.attackers.length > 0) {
        ...
        // Keep state consistent if combat is empty.
